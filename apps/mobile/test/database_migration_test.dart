...
void main() {
  test('schema v1 to v10 preserves its account and conversation', () async {
    final fixture =
        readFixtureJson(
  ...
  test(
    'schema v2 to v10 preserves rooms and backfills avatar metadata',
    () async {
      final fixture =
          readFixtureJson(
                'conversation-list/fixtures/conversations-full.response.json',
  ...
  test('schema v3 to v10 clears avatars lacking custom metadata', () async {
    final database = AppDatabase.forTesting(
      NativeDatabase.memory(
        setup: (raw) {
          raw
  ...
  test(
    'schema v4 to v10 preserves outbox rows with no thread binding',
    () async {
      final database = AppDatabase.forTesting(
        NativeDatabase.memory(
          setup: (raw) {
  ...
  test(
    'schema v5 to v10 preserves thread binding and recovers sending rows',
    () async {
  ...
  test('schema v6 to v10 preserves chat rows and creates join index', () async {
    final directory = await Directory.systemTemp.createTemp(
  ...
  test('schema v7 to v10 adds nullable peer status columns', () async {
    final directory = await Directory.systemTemp.createTemp(
  ...
  test('schema v8 to v10 creates the durable draft table', () async {
    final directory = await Directory.systemTemp.createTemp(
  ...
  test('schema v9 to v10 backfills isArchived from raw_json', () async {
    final directory = await Directory.systemTemp.createTemp(
