...
void main() {
  ...
  testWidgets('a compact shell hands the open conversation to the navigator', (
    tester,
  ) async {
    const account = StoredAccount(
      id: 'account-a',
    ...
    // The compact shell has no second pane, so a conversation selected while
    // the window was wide would be lost. It has to reach the navigator.
    expect(find.byKey(const Key('conversation-shell-compact')), findsOneWidget);
    expect(opened, <String>[conversation.token]);

    // Staying compact must not push the same conversation again.
    await tester.pump();
    await tester.pump();
    expect(opened, <String>[conversation.token]);
