...
/// Drives `talk_protocol`'s push-v2 state machine for every iOS account:
/// device key, Nextcloud registration, and the `nks-talk-notify` proxy
/// registration that lets APNs actually reach the app while it is closed.
/// See `nks-talk-notify/README.md` for the wire contract each effect below
/// executes.
///
/// The state machine itself (`packages/talk_protocol/lib/src/push`) is pure:
/// [planNextPushEffect] says what to do next, this coordinator does it (a
/// Keychain call or an HTTP request), and [completePushEffect] folds the
/// result back in. Only one effect is ever in flight — the state machine
/// hands out at most one pending effect at a time — so the drive loop below
/// needs no concurrency of its own.
final class ApplePushRegistrationCoordinator {
  ApplePushRegistrationCoordinator({
  ...
  /// Feeds a freshly issued (or refreshed) APNs device token in. Every
  /// account still needing to register picks this up on the next drain.
  void installToken(String hexToken) {
    if (_disposed) {
      return;
    }
    _providerGeneration++;
    final generation = _providerGeneration;
    final handle = PushTokenHandle.parse('apns-token-$generation');
    // The proxy's `pushTokenHash` contract hashes the UTF-8 hex string, not
    // the raw token bytes — see nks-talk-notify/README.md, "client contract
    // that is not visible in this repo".
    final sha512Hex = crypto.sha512.convert(utf8.encode(hexToken)).toString();
    _rawToken = hexToken;
    _apply(
