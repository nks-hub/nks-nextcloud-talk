...
final class _RichDocument extends StatelessWidget {
  const _RichDocument({required this.document, required this.foregroundColor});
  ...
  Widget _buildBlock(BuildContext context, RichChatSemanticNode node) {
    ...
    return switch (node.kind) {
      RichChatSemanticKind.heading => Text.rich(
        TextSpan(children: _inlineChildren(context, node, foregroundColor)),
        style: _headingStyle(
          theme,
          node.headingLevel ?? 3,
...
final class _RichList extends StatelessWidget {
  const _RichList({required this.node, required this.foregroundColor});
...
final class _RichTable extends StatelessWidget {
  const _RichTable({required this.node, required this.foregroundColor});
...
List<InlineSpan> _inlineChildren(
  ...
  if (node.children.isEmpty) {
    return [_inlineSpan(context, node, foregroundColor)];
  }
  return node.children
      .map((child) => _inlineSpan(context, child, foregroundColor))
      .toList();
}
