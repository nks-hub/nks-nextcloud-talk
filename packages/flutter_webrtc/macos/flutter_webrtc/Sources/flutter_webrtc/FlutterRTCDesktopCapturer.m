...
- (void)getDisplayMedia:(NSDictionary*)constraints result:(FlutterResult)result {
  ...
  void (^publishStream)(void) = ^{
    RTCVideoTrack *track = [self.peerConnectionFactory videoTrackWithSource:videoSource
                                                                 trackId:trackUUID];
    [mediaStream addVideoTrack:track];
    self.localTracks[trackUUID] = [[LocalVideoTrack alloc] initWithTrack:track
                                                     videoProcessing:videoProcessingAdapter];
    self.localStreams[mediaStreamId] = mediaStream;
    result(@{@"streamId": mediaStreamId, @"audioTracks": @[], @"videoTracks": @[@{
      @"id": track.trackId, @"kind": track.kind, @"label": track.trackId,
      @"enabled": @(track.isEnabled), @"remote": @YES, @"readyState": @"live"
    }]});
  };
  ...
  */
  ...
  if (useScreenCaptureKit) {
    ...
    if (@available(macOS 13.0, *)) {
                                          ...
                                          onStarted:^(NSError * _Nullable error) {
                                            dispatch_async(dispatch_get_main_queue(), ^{
                                              if (error != nil) {
                                                ...
                                                result([FlutterError errorWithCode:@"NotReadableError"
      ...
      desktopCapturer = [self legacyCapturerWithSource:source
                                       captureDelegate:videoProcessingAdapter];
    }
  }
