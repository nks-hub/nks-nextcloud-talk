...
void main() {
  test('rich-chat runtime executes as a release AOT executable', () async {
    final packageRoot = Directory.current.absolute;
    final probe = File(
      '${packageRoot.path}/test/support/rich_chat_release_probe.dart',
    );
    expect(probe.existsSync(), isTrue);

    final temporaryDirectory = await Directory.systemTemp.createTemp(
      'talk_protocol_rich_chat_',
    );
    try {
      final executable = File(
        '${temporaryDirectory.path}/rich_chat_release_probe'
